...
// Faixas de INSS conforme legislação vigente
...
const calcularIRRF = (baseCalculo: number) => {
  ...
  const irrfSimplificado = calcularImpostoPuro(Math.max(0, baseCalculo - 564.80));
...
const FormInput = ({ label, type = "text", className = "", options, ...props }: any) => (
  <div className={`mb-1.5 ${className}`}>
    <label className="block text-[8px] font-bold text-slate-400 mb-0.5 uppercase tracking-wider">{label}</label>
    {options ? (
      <div className="relative">
        <select 
          className="w-full px-2 py-1 bg-slate-900 border border-slate-700 rounded focus:ring-1 focus:ring-emerald-500 outline-none text-slate-200 text-[10px] font-medium appearance-none"
          {...props}
        >
          {options.map((opt: any) => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
        </select>
        <div className="absolute right-2 top-1 pointer-events-none text-emerald-500">
      ...
      <input 
        type={type}
        className="w-full px-2 py-1 bg-slate-900 border border-slate-700 rounded focus:ring-1 focus:ring-emerald-500 outline-none text-slate-200 placeholder-slate-600 text-[10px] font-medium"
        {...props}
      />
    )}
  </div>
...
const ResultCard = ({ title, value, subtext, highlight = false, onClick }: any) => (
  <div onClick={onClick} className={`bg-slate-900 p-3.5 rounded-xl border ${highlight ? 'border-emerald-500/50 ring-1 ring-emerald-500/20 shadow-emerald-900/10 shadow-lg' : 'border-slate-800'} ${onClick ? 'cursor-pointer hover:border-emerald-500/50 transition-all' : ''}`}>
    <div className="text-[8px] font-bold text-slate-500 uppercase tracking-widest mb-0.5 flex items-center gap-2">
        {title}
        {onClick && <span className="material-icons-round text-[8px] text-emerald-500">edit</span>}
    </div>
    <div className={`text-lg font-black ${highlight ? 'text-emerald-400' : 'text-slate-200'} font-mono`}>{value}</div>
    {subtext && <div className="text-[8px] text-slate-500 mt-0.5 font-bold">{subtext}</div>}
  </div>
);
...
const LineItem: React.FC<LineItemProps> = ({ label, value, subtext, type = 'neutral' }) => {
    ...
    return (
        <div className="flex justify-between items-center py-1.5 border-b border-slate-800 last:border-0 hover:bg-slate-800/30 transition-colors px-1.5 rounded-lg group">
            <div>
            ...
            <span className={`text-[11px] font-mono font-black ${type === 'plus' ? 'text-emerald-400' : type === 'minus' ? 'text-rose-500' : 'text-slate-400'}`}>
...
function App() {
                        ...
                        <div className="flex justify-between items-end border-b-2 border-slate-800 pb-2 mb-4">
                            <div>
                                <div className="flex items-center gap-2 mb-1">
                                    <span className="material-icons-round text-3xl text-emerald-600">calculate</span>
  ...
  return (
    <div className="min-h-screen bg-[#020617] font-sans text-slate-300 pb-10">
      <div className="max-w-6xl mx-auto p-4 md:p-8 no-print">
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-start">
            <div className="w-full lg:w-1/3 bg-slate-900/50 backdrop-blur-sm p-3.5 rounded-2xl shadow-xl border border-slate-800 h-fit sticky top-4">
                <div className="mb-2">
                    <label className="block text-[8px] font-bold text-slate-400 mb-0.5 uppercase tracking-wide">Motivo da Rescisão</label>
                    <div className="flex bg-slate-950 p-0.5 rounded-lg border border-slate-800">
                        <button onClick={() => setFormData(prev => ({ ...prev, motivo: 'dispensa' }))} className={`flex-1 py-1 text-[9px] font-bold rounded transition-all uppercase tracking-wide ${formData.motivo === 'dispensa' ? 'bg-emerald-500 text-slate-950 shadow-md shadow-emerald-900/20' : 'text-slate-500 hover:text-slate-300'}`}>Demitido</button>
                        <button onClick={() => setFormData(prev => ({ ...prev, motivo: 'pedido' }))} className={`flex-1 py-1 text-[9px] font-bold rounded transition-all uppercase tracking-wide ${formData.motivo === 'pedido' ? 'bg-emerald-500 text-slate-950 shadow-md shadow-emerald-900/20' : 'text-slate-500 hover:text-slate-300'}`}>Pedido</button>
                    </div>
                </div>
                ...
                <div className="mt-4">
                    <button onClick={handleCalcular} className="w-full bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black py-2.5 rounded-xl shadow-lg shadow-emerald-900/20 transition-all flex justify-center items-center gap-2 text-[10px] uppercase tracking-widest transform active:scale-[0.98]"><span className="material-icons-round text-lg">play_arrow</span> Calcular Rescisão</button>
                </div>
            </div>

            <div className="w-full lg:w-2/3">
                {!calculo ? (
                    <div className="bg-slate-900/30 rounded-2xl border-2 border-dashed border-slate-800 h-full min-h-[440px] flex flex-col items-center justify-center text-slate-600">
                    ...
                    <div className="space-y-4 animate-fade-in">
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <ResultCard title="Líquido a Receber" value={formatCurrency(calculo.rescisaoLiquida)} subtext="Rescisão disponível" />
                        </div>
                        ...
                        <div className="bg-slate-900 rounded-xl shadow-lg border border-slate-800 overflow-hidden relative">
                             ...
                             <div className="px-5 py-3 flex justify-between items-center border-b border-slate-800/50 bg-slate-950/30">
                                ...
                                <button onClick={() => setShowFGTSModal(true)} className="text-[8px] font-black text-emerald-500 bg-emerald-500/10 hover:bg-emerald-500/20 px-2.5 py-1.5 rounded uppercase tracking-wider transition-colors border border-emerald-500/20">Editar Saldo</button>
                             </div>
                             {calculo.isPedidoDemissao ? (
                                <div className="p-5 text-xs text-slate-500 italic text-center bg-slate-950/20">Sem saque de FGTS para pedidos de demissão.</div>
                             ) : (
                                <div className="p-4 space-y-2 relative z-10">
                                    <div className="flex justify-between items-center text-xs">
                                        <div><div className="text-slate-400 font-bold text-[9px] uppercase">Saldo Base</div><div className="text-[8px] text-slate-600 font-bold">Fins Rescisórios</div></div>
                                        <div className="font-mono font-bold text-slate-300">{formatCurrency(calculo.saldoFGTSBase + calculo.fgtsRescisao + calculo.fgtsAvisoIndenizado)}</div>
                                    </div>
                                    <div className="flex justify-between items-center text-xs border-b border-slate-800 pb-2">
                                        <div className="text-slate-400 font-bold text-[9px] uppercase">Multa 40%</div>
                                        <div className="font-mono font-bold text-emerald-500">{formatCurrency(calculo.multa40)}</div>
                                    </div>
                                    <div className="flex justify-between items-center pt-1">
                                        <div className="text-slate-200 font-black text-xs uppercase tracking-wide">Total Saque FGTS</div>
                                        <div className="font-mono font-black text-emerald-400 text-lg">{formatCurrency(calculo.totalContaFGTS)}</div>
                                    </div>
                                </div>
                             )}
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="bg-slate-900 rounded-xl border border-slate-800 shadow-lg flex flex-col">
                                <div className="px-5 py-2.5 flex justify-between items-center border-b border-slate-800 bg-slate-950/30">
                                    <div className="flex items-center gap-2"><span className="material-icons-round text-emerald-500 text-sm">add_circle</span><span className="text-[9px] font-black text-emerald-500 uppercase tracking-wide">Proventos</span></div>
                                    <button onClick={() => setShowAdjustModal(true)} className="text-[8px] font-bold text-slate-500 hover:text-white uppercase tracking-wide transition-colors">Lançar</button>
                                </div>
                                <div className="p-2 flex-grow">
                                    <LineItem label="Saldo de Salário" value={calculo.saldoSalario} subtext={`${calculo.diasTrabalhados} dias`} type="plus" />
                                    <LineItem label="Aviso Prévio" value={calculo.valorAviso} type="plus" />
                                    <LineItem label="13º Salário Prop." value={calculo.valor13} subtext={`${calculo.avos13}/12`} type="plus" />
                                    {calculo.valor13Indenizado > 0 && <LineItem label="13º s/ Aviso" value={calculo.valor13Indenizado} type="plus" />}
                                    <LineItem label="Férias Vencidas" value={calculo.valorFeriasVencidas + calculo.tercoFeriasVencidas} type="plus" />
                                    {calculo.valorFeriasDobro > 0 && <LineItem label="Férias Dobro" value={calculo.valorFeriasDobro + calculo.tercoFeriasDobro} type="plus" />}
                                    <LineItem label="Férias Prop." value={calculo.valorFeriasProp + calculo.tercoFeriasProp} type="plus" />
                                    {ajustes.filter(a => a.tipo === 'Provento').map((aj, idx) => <LineItem key={`aj-p-${idx}`} label={aj.descricao} value={aj.valor} type="plus" />)}
                                </div>
                            </div>
                            
                            <div className="bg-slate-900 rounded-xl border border-slate-800 shadow-lg flex flex-col">
                                <div className="px-5 py-2.5 flex justify-between items-center border-b border-slate-800 bg-slate-950/30">
                                    <div className="flex items-center gap-2"><span className="material-icons-round text-rose-500 text-sm">remove_circle</span><span className="text-[9px] font-black text-rose-500 uppercase tracking-wide">Descontos</span></div>
                                </div>
                                <div className="p-2 flex-grow">
                                    <LineItem label="INSS" value={calculo.descontoINSS} type="minus" />
                                    <LineItem label="IRRF" value={calculo.totalIRRF} type="minus" />
                                    {calculo.valorAvisoDesconto > 0 && <LineItem label="Aviso Prévio (Desc)" value={calculo.valorAvisoDesconto} type="minus" />}
                                    {ajustes.filter(a => a.tipo === 'Desconto').map((aj, idx) => <LineItem key={`aj-d-${idx}`} label={aj.descricao} value={aj.valor} type="minus" />)}
                                </div>
                            </div>
                        </div>

                        <div className="flex flex-col sm:flex-row gap-3 pt-6 justify-center">
                             <button onClick={() => setShowAdjustModal(true)} className="text-slate-300 bg-slate-900 border border-slate-700 hover:bg-slate-800 px-6 py-2.5 rounded-xl font-bold text-[9px] uppercase tracking-wide flex items-center justify-center gap-2 transition-all">
                                 <span className="material-icons-round text-base">post_add</span> Lançamento Manual
                             </button>
                             <button onClick={togglePrintPreview} className="text-slate-950 bg-emerald-500 hover:bg-emerald-400 px-8 py-2.5 rounded-xl font-black text-[9px] uppercase tracking-wide flex items-center justify-center gap-2 transition-all shadow-lg shadow-emerald-900/20">
                                 <span className="material-icons-round text-base">description</span> Imprimir Demonstrativo
                             </button>
                        </div>
                    </div>
                )}
      ...
      {/* MODAL FGTS */}
      {showFGTSModal && (
        <div className="fixed inset-0 bg-slate-950/90 z-[9999] flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in no-print">
            <div className="bg-slate-900 rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden animate-slide-up border border-slate-800">
                ...
                <div className="p-5 overflow-y-auto custom-scrollbar bg-slate-900">
                    <div className="mb-4 bg-slate-950 p-4 rounded-2xl border border-slate-800">
                        ...
                        <input type="number" className="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-xl focus:ring-1 focus:ring-emerald-500 outline-none text-white text-xs font-mono" placeholder="0.00" value={fgtsSaldoManual} onChange={(e) => setFgtsSaldoManual(Number(e.target.value))} />
                    </div>
                    <div className="flex justify-between items-end mb-3"><h4 className="font-bold text-slate-400 text-[10px] uppercase tracking-wide">Lançamentos Mensais (8%)</h4><button onClick={preencherSalarioMinimo} className="text-[9px] font-bold text-emerald-500 hover:bg-emerald-500/10 px-2.5 py-1 rounded transition-colors uppercase border border-emerald-500/20">Preencher com Mínimo</button></div>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">{fgtsManualData.map((item, idx) => (<div key={idx} className="bg-slate-950 p-1.5 rounded-xl border border-slate-800"><label className="block text-[9px] font-bold text-slate-500 mb-0.5 uppercase text-center">{item.date}</label><input type="number" className="w-full px-1.5 py-1 bg-slate-900 border border-slate-700 rounded-lg text-[10px] text-white text-center focus:border-emerald-500 outline-none transition-all font-mono" value={item.value} onChange={(e) => updateFgtsValue(idx, Number(e.target.value))} /></div>))}</div>
                </div>
                <div className="p-4 border-t border-slate-800 bg-slate-900 flex justify-end gap-2.5"><button onClick={() => setShowFGTSModal(false)} className="px-4 py-2 text-slate-500 font-bold hover:text-white transition-colors text-[10px] uppercase tracking-wide">Cancelar</button><button onClick={() => { handleCalcular(); setShowFGTSModal(false); }} className="px-5 py-2 bg-emerald-500 text-slate-950 rounded-xl hover:bg-emerald-400 font-black shadow-lg shadow-emerald-900/20 text-[10px] uppercase tracking-wide">Salvar</button></div>
            </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-950/80 z-[9998] flex items-center justify-center p-4 backdrop-blur-md no-print">
            <div className="bg-slate-900 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-slide-up border border-slate-800">
                ...
                <form onSubmit={addAjuste} className="p-5 space-y-3">
                    <div><label className="block text-[10px] font-bold text-slate-400 mb-1 uppercase">Descrição</label><input name="descAjuste" required className="w-full bg-slate-950 border border-slate-700 px-3 py-2 rounded-xl focus:ring-1 focus:ring-emerald-500 outline-none text-white text-xs" placeholder="Ex: Horas Extras..." /></div>
                    <div><label className="block text-[10px] font-bold text-slate-400 mb-1 uppercase">Valor (R$)</label><input name="valAjuste" type="number" step="0.01" required className="w-full bg-slate-950 border border-slate-700 px-3 py-2 rounded-xl focus:ring-1 focus:ring-emerald-500 outline-none text-white text-xs font-mono" placeholder="0.00" /></div>
                    <div><label className="block text-[10px] font-bold text-slate-400 mb-1 uppercase">Tipo</label><select name="tipoAjuste" className="w-full bg-slate-950 border border-slate-700 px-3 py-2 rounded-xl outline-none text-white text-xs font-medium"><option value="Provento">Provento (+)</option><option value="Desconto">Desconto (-)</option></select></div>
                    <button type="submit" className="w-full bg-emerald-500 hover:bg-emerald-400 text-slate-950 py-2.5 rounded-xl font-black mt-1 text-[10px] uppercase tracking-widest">Adicionar Item</button>
                </form>
                <div className="px-5 pb-5">
                    <h4 className="font-bold text-[8px] uppercase text-slate-600 mb-2 tracking-widest">Itens Adicionados</h4>
                    {ajustes.length === 0 ? <div className="text-[10px] text-slate-600 italic text-center py-3 bg-slate-950 rounded-xl border border-dashed border-slate-800">Nenhum lançamento.</div> : (
                        <ul className="space-y-1 max-h-32 overflow-y-auto pr-1 custom-scrollbar">
                            {ajustes.map((aj, i) => (<li key={i} className="flex justify-between items-center text-[10px] bg-slate-950 p-2 rounded-lg border border-slate-800"><span className="font-bold text-slate-300 uppercase truncate max-w-[150px]">{aj.descricao}</span><div className="flex items-center gap-2"><span className={`font-black font-mono ${aj.tipo === 'Provento' ? 'text-emerald-500' : 'text-rose-500'}`}>{aj.tipo === 'Provento' ? '+' : '-'} {formatCurrency(aj.valor)}</span><button onClick={() => setAjustes(ajustes.filter((_, idx) => idx !== i))} className="text-slate-600 hover:text-rose-500"><span className="material-icons-round text-sm">delete</span></button></div></li>))}
                        </ul>
                    )}
                    <div className="mt-4 pt-3 border-t border-slate-800"><button onClick={() => { handleCalcular(); setShowAdjustModal(false); }} className="w-full py-2 bg-slate-800 text-slate-400 rounded-xl hover:text-white font-black text-[9px] uppercase transition-colors">Fechar</button></div>
                </div>
            </div>
        </div>
      )}
